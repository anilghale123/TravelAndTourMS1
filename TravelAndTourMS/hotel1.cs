...
{
    public partial class hotel1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            // Create a new ComboBox to hold the places
            ComboBox comboBox = new ComboBox();
            comboBox.Items.Add("Kathmandu");
            comboBox.Items.Add("Pokhara");
            comboBox.Items.Add("Rasuwa");
            comboBox.Items.Add("Chitwan");

            // Set the ComboBox's location and size
            comboBox.Location = new Point(10, 10);
            comboBox.Size = new Size(150, 20);

            // Add the ComboBox to the form's Controls collection
            this.Controls.Add(comboBox);

            // Create a new Button to filter the hotels
            Button button = new Button();
            button.Text = "Filter Hotels";
            button.Location = new Point(170, 10);

            // Attach an event handler to the Button's Click event
            // button.Click += new EventHandler(FilterHotels);
            ...
            using (SqlConnection connection = new SqlConnection(con.ConnectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("SELECT * FROM Hotel WHERE place = @place", connection))
                {
                    cmd.Parameters.AddWithValue("place", comboBox1.Text);
